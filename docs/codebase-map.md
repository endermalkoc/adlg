...
- **beads** — `/home/ender/repos/misc/beads` (`../../misc/beads` from this repo). The MIT project
  ASDF's Dolt infrastructure was **salvaged and adapted** from; [NOTICE](../NOTICE) lists exactly
  which packages were lifted. When ROADMAP says "adapt beads' X" or "beads has Y", that's where it
  lives. Treat as a read-only reference — ASDF's `internal/` copies have rewritten import paths and
...
| Path | What it is |
|---|---|
| `cmd/asdf/` | the CLI (cobra). `main.go` → `root.go` (flags, `connect`, `emit`); one file per group: `init`, `domain`, `spec`, `req`, `edge`, `changeset` |
| `internal/app/` | **`Mutate`** — the command-contract wrapper **every** mutating command routes through; `validate.go` |
| `internal/workspace/` | `.asdf` resolution, managed connect (owned/external/embedded), `ResolveActor`, active-changeset state, `WithRetryTx`, `Diff` |
...
| Roadmap item | Start in ASDF | beads reference |
|---|---|---|
| Remote sync (`asdf dolt push/pull`) | `versioncontrolops/remotes.go`, `remotecache` | `cmd/bd/dolt.go`, `federation.go` |
...
| Agent setup (Claude Code, …) | net-new `cmd/asdf` + a `setup` pkg | `cmd/bd/setup/{claude,codex,cursor,…}.go` |
...
1. New file in `cmd/asdf/` with a cobra command.
...
the `dolt` binary on PATH; `asdf init` bootstraps a workspace, then the verbs work against the
managed server.
