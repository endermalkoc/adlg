...
// readVerbs are the statement kinds `asdf sql` permits. Writes are rejected so the database
...
var sqlCmd = &cobra.Command{
	...
	Long: "Execute a read-only SQL query against the canonical Dolt database and print the result.\n" +
		...
		"system tables (e.g. `asdf sql \"SELECT * FROM dolt_log LIMIT 5\"`). Use `-` to read from stdin.",
...
// ensureReadOnly rejects any statement that is not a read, so `asdf sql` can never bypass the
...
func ensureReadOnly(query string) error {
	...
	if !readVerbs[verb] {
		return app.ValidationFailed(fmt.Errorf("asdf sql is read-only (got %q) — use the entity commands to write", verb))
	}
	return nil
}
